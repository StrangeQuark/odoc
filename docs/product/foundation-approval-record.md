...
**Status:** pending project-owner decisions and operational contact setup
...
or approval of only one pull request. The project owner must approve each decision or
record a replacement before `P0-001` is complete.
...
| `FND-002` | License Odoc source, tests, build/deploy definitions, and project documentation as `AGPL-3.0-or-later` | Apache-2.0/MPL-2.0 weaken network copyleft; `AGPL-3.0-only` removes the later-version option; dual licensing requires different rights/governance | Pending |
...
| `FND-005` | Adopt the factual-use and official-channel rules in [TRADEMARKS.md](../../TRADEMARKS.md), reject “Odoc” as the public name after the [collision screen](../project/name-collision-research.md), and select one replacement candidate for expanded availability and qualified legal review. The expanded screen rejected Tetherleaf, Quirelia, and Arborquill; fresh uncleared finalists are Schemalyth (current recommendation for the next review stage) and Dovureli, while an owner-proposed alternative receives the same screening. Candidate selection is not public-name adoption, and `P0-002` may not fix packages/artifacts/runtime branding until the selected candidate's qualified legal review is recorded and the owner separately approves adoption. Current repository names remain temporary locators until a separately authorized migration | Override the Odoc rejection only with documented qualified legal clearance addressing the active same-field OCaml tool and other exact-name records; do not treat a knockout screen as clearance | Pending |
| `FND-006` | Require external OIDC for initial production identity; defer native passwords, recovery, and MFA to the separately gated `P5-531` decision | Accept the materially larger credential-security scope before 1.0 | Pending |
...
| `FND-014` | Adopt the AGPL-compatible dependency allow/manual-review/deny policy, exact artifact-to-source mapping, notices/SBOM expectations, and no required proprietary/premium add-on | Replace the allowlist/exception process before any dependency is selected | Pending |
...
Policy text alone does not make an intake channel operational. These items require
current-state evidence and are not satisfied by approving the table above.
...
| `OPS-004` | Exact unmodified GNU AGPL v3 text in root `LICENSE`, with repository metadata consistent with `AGPL-3.0-or-later` | Intentionally absent until `FND-002` approval |
...
> I approve `FND-001` through `FND-019` as written in the 2026-08-14 foundation
...
> I select **________** only as a replacement candidate for expanded availability and
> qualified legal review; I do not yet approve it as the public name. I understand
...
The candidate line authorizes expanded screening only. Even after the foundation-policy
approval, `P0-001` remains incomplete until the qualified legal review and a separate
owner adopt/replace decision are recorded; `P0-002` cannot freeze public branding
sooner.
...
| Selected replacement candidate, qualified-legal-review reference, and later adoption decision | Pending |
