...
| `FND-006` | Require built-in email/password accounts for MVP, with verified email, secure sessions, recovery, throttling, and audit. The approved local/private profile allows normal self-service registration; a future deployment may deliberately select invite-only or allowlist enrollment. OIDC/OAuth/SSO remain optional additional sign-in methods; MFA/passkeys move to the separately gated `P5-531` hardening package | Accept the credential-security scope now, rather than making an external IdP a self-hosting prerequisite | Approved by owner 2026-08-15; enrollment policy clarified 2026-08-18 |
