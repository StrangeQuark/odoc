...
class PageCommentService {
    private final PageCommentRepository comments; private final PageRepository pages; private final Clock clock;
    @Autowired
    PageCommentService(PageCommentRepository comments, PageRepository pages) { this(comments, pages, Clock.systemUTC()); }
    PageCommentService(PageCommentRepository comments, PageRepository pages, Clock clock) { this.comments = comments; this.pages = pages; this.clock = clock; }
    ...
    private void requirePage(UUID pageId) { if (!pages.existsById(pageId)) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Page not found."); }
}
